...
const NewTrail = () => {
    ...
    const [tempTrailObj, setTempTrailObj] = useState({ image: [], marker: [{ latlong: { latitude: 0, longitude: 0 } }], star: [], time: "", trafficStart: [], trafficEnd: [], xlabel: [], ylabel: [], path: [] })
    ...
    const preview = async () => {
        ...
        markerArr = [{ latlong: { latitude: pathArr[0].latitude, longitude: pathArr[0].longitude }, title: "" } , ...markerArr, { latlong: { latitude: pathArr[pathArr.length - 1].latitude, longitude: pathArr[pathArr.length - 1].longitude }, title: "" }]
    ...
    const create = async () => {
        ...
        await API.createTrail(tempTrailObj).then(([code, data, header]) => {
            if (code == '200') {
                const trailData = JSON.parse(localStorage.getItem('trailData'))
                const newTrailData = [...trailData, data]
                localStorage.setItem('trailData', JSON.stringify(newTrailData))
            }
        })
            ...
            <div className="newTrail">
                ...
                <div className="newTrailContainer">
                    ...
                    >
                        ...
                        <div className="right" style={{ display: 'flex', flexWrap: 'wrap', gap: 20, alignItems: 'center', justifyContent: 'center' }}>

                            <div style={{ width: '40%' }}>
                                <label>Territory:</label>
                                <p style={{ padding: 5, borderBottom: '1px solid gray' }}>{tempTrailObj.district}</p>
                            </div>
                            <div style={{ width: '40%' }}>
                                <label>District:</label>
                                <p style={{ padding: 5, borderBottom: '1px solid gray' }}>{tempTrailObj.place}</p>
                            </div>
                            <div style={{ width: '40%' }}>
                                <label>Trail Name:</label>
                                <p style={{ padding: 5, borderBottom: '1px solid gray' }}>{tempTrailObj.title}</p>
                            </div>
                            <div style={{ width: '40%' }}>
                                <label>Description:</label>
                                <p style={{ padding: 5, borderBottom: '1px solid gray' }}>{tempTrailObj.description}</p>
                            </div>
                            <div style={{ width: '40%' }}>
                                <label>Distance (km):</label>
                                <p style={{ padding: 5, borderBottom: '1px solid gray' }}>{tempTrailObj.distance}</p>
                            </div>
                            <div style={{ width: '40%' }}>
                                <label>Difficulty (1-5 stars):</label>
                                <p style={{ padding: 5, borderBottom: '1px solid gray' }}>{tempTrailObj.star.filter((star) => { return star == '1' }).length}</p>
                            </div>
                            <div style={{ width: '40%' }}>
                                <label>Time:</label>
                                <p style={{ padding: 5, borderBottom: '1px solid gray' }}>{tempTrailObj.time.split(".")[0] == '0' ?
                            ...
                            <div className="mapContainer" style={{ zIndex: 0, width: '60%' }}>
                                <p>Map: </p>
                                <MapContainer center={[tempTrailObj.marker[0].latlong.latitude, tempTrailObj.marker[0].latlong.longitude]} zoom={15} style={{ height: 500, width: '100%', border: 'none' }}>
                                    <TileLayer
                                    ...
                                    />
                                    {
                                        tempTrailObj.marker.map((marker, index) => (
                                            <Marker position={[marker.latlong.latitude, marker.latlong.longitude]} icon={myIcon} key={index}>
                                                <Tooltip permanent>
                                                    {index == 0 ? "起點" : index == tempTrailObj.marker.length - 1 ? "終點" : `${marker.title}`}
                                                </Tooltip>
                                            </Marker>
                                        ))
                                    }
                                    <Polyline pathOptions={{ color: 'black' }} positions={tempTrailObj.path.map((path) => { return [path.latitude, path.longitude] })} />
                                </MapContainer>
                            </div>
                            <div style={{ width: '60%' }}>
                            ...
                            {isLoadingCreate ?
                                <div style={{ width: '60%', alignItems: 'center', justifyContent: 'center', display: 'flex', marginTop: 20, marginBottom: 20 }}>
                                    <ReactLoading type="spin" color="teal" />
                                </div>
                                :
                                <div style={{ width: '60%', alignItems: 'center', justifyContent: 'center', display: 'flex', marginTop: 20, marginBottom: 20, gap: 20 }}>
                                    <button onClick={() => { create() }} style={{ width: '50%', padding: 10, border: 'none', backgroundColor: 'teal', color: 'white', 'fontWeight': 'bold', cursor: 'pointer', borderRadius: 5, marginTop: 20, marginBottom: 20 }}>Create</button>
                                    <button onClick={() => { setIsModalOpen(false) }} style={{ width: '50%', padding: 10, border: 'none', backgroundColor: '#de4949', color: 'white', 'fontWeight': 'bold', cursor: 'pointer', borderRadius: 5, marginTop: 20, marginBottom: 20 }}>Go Back</button>
                                </div>
                            }
